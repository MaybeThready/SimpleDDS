...
发送完命令控制字后，如果未选择不发送数据，则要紧接着发送数据控制字，格式如下
...
我们发现将复杂的逻辑交给上位机处理似乎会更好一些，因此我们设计了第二版控制字，减小下位机的处理压力。

我们约定：一次数据传送总是发送10个字节数据，分别是命令控制字（1字节），freq1（3字节），freq2（3字节），param（3字节）。其中，依据不同的工作方式，freq1分别代表基础频率/载波频率，freq2分别代表调制波频率/数字信号速率（低16位），param分别代表波形类型（低8位）/调制度（低8位）或频率偏移（24位）/数字信号数据（低8位）。

最后，为了方便解析，我们约定所有发送的数据总是先发送低字节，再发送高字节，例如频率为 0x123456 Hz 的话，应该先发送 0x56，再发送 0x34，最后发送 0x12。
